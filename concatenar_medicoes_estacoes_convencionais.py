...
def concatenar_arquivos_csv(nome, prefixo, skiprows, usecols=None, date_columns=None, sort_columns=None, output_filename=None):
    print(nome)
    print(f'\tLendo...')
    ...
    if len(files) > 0:
        print('\tConcatenando...')
        dfs = [pd.read_csv(f, sep=';', skiprows=skiprows, index_col=False, dayfirst=True, parse_dates=date_columns, decimal=',', usecols=usecols, encoding='iso-8859-1') for f in files]
        ...
        concatenated_df.to_csv(f'{output_folder}/{prefixo}/{output_filename}.csv', index=False, encoding='utf-8-sig')
    else:
...
concatenar_arquivos_csv(nome='CURVA DESCARGA', prefixo='curvadescarga', skiprows=12, usecols=range(0, 17), sort_columns=['EstacaoCodigo', 'NivelConsistencia', 'PeriodoValidadeInicio', 'PeriodoValidadeFim'], output_filename='curvadescarga')
concatenar_arquivos_csv(nome='PERFIL TRANSVERSAL', prefixo='PerfilTransversal', skiprows=11, date_columns=[2, 3], usecols=range(0, 15), sort_columns=['EstacaoCodigo', 'Data', 'Hora'], output_filename='perfiltransversal')
